...
Git 源码树只包含文档、元数据和只读工具，不包含固件二进制，也不提供自动刷写
功能。经维护者确认拥有公开分发授权后，`v231227-r2` Release 单独提供两个未经
修改的 V231227 参考镜像。
...
仓库原创文档和工具采用 MIT 许可。Release 中的两个 OEM 镜像不适用 MIT
许可，其商标、代码和签名材料仍归各自权利人所有；本次发布不构成对其他 OEM
固件或设备唯一数据的分发授权。
